...
#[defmt_test::tests]
mod tests {
    use p256_cm4::{Signature, VerifyingKey, convert_endianness};
    ...
    fn test_verify_new() {
        ...
        let key =
            VerifyingKey::from_parts(u32x8_to_u8x32(&CURVE_PT_X), u32x8_to_u8x32(&CURVE_PT_Y))
                .unwrap();
        let signature =
            Signature::from_parts(u32x8_to_u8x32(&R_SIGN), u32x8_to_u8x32(&S_SIGN)).unwrap();

        let authentic = key.verify_prehash(&hash, &signature);

        let elapsed: u32 = DWT::cycle_count().wrapping_sub(start);

        defmt::info!("Approximate cycles per p256 verify (new API): {}", elapsed);
